<?php
	...
	{
		while($cate = mysql_fetch_array($result, MYSQL_ASSOC))
		{
			$cates[] = $cate;
		}
	}
	...
	{
		$cates = null;
		alertMes("对不起，网站维护中", "www.baidu.com") ;
	}
?>
<!doctype html>
<html>
...
<div class="shopList comWidth clearfix">
	...
	<div class="rightArea">
		<div class="shopList_top clearfix">
			<?php
				$catePros = getFourProsByCate($cate['id']);
				//print_r($catePros);
				if(is_array($catePros) && count($catePros > 0)){
					foreach($catePros as $catePro)
					{
						$proImg = getProImageById($catePro['id']);
						if($proImg && is_array($proImg))
						{
							$imgPath = "image_220/". $proImg['albumPath'];
						}
						else
						{
							$imgPath = "images/shopImg.jpg";
						}
						//print_r($catePro);
			?>
			<div class="shop_item">
				<div class="shop_img">
					<a href="proDetail.php?id=<?php echo $catePro['id']; ?>">
						<img height="200" width="187" src="<?php echo $imgPath;?>"
						alt=""></a>
				</div>
				<h3><?php echo $catePro['pName'];?></h3>
				<p><?php echo $catePro['pPrice'];?></p>
			</div>
				<?php }
				}?>
		</div>
		<div class="shopList_sm clearfix">
			<?php
				$cateSmallPros = getNextFourProsByCate($cate['id']);
				//print_r($catePros);
				if(is_array($cateSmallPros) && count($cateSmallPros > 0)){
					foreach($cateSmallPros as $cateSmallPro)
					{
						$proImg = getProImageById($cateSmallPro['id']);
						if($proImg && is_array($proImg))
						{
							$imgPath = "image_220/". $proImg['albumPath'];
						}
						...
						//print_r($cateSmallPro);
			?>
			<div class="shopItem_sm">
				<div class="shopItem_smImg">
					<a href="proDetail?id=<?php echo $cateSmallPro['id'];?>">
						<img height="95" width="95" src="<?php echo $imgPath;?>"
						alt=""></a>
				</div>
				<div class="shopItem_text">
					<p><?php $cateSmallPro['pName'];?></p>
					<h3><?php $cateSmallPro['pPrice'];?></h3>
				</div>
			</div>
					<?php }
				}?>
